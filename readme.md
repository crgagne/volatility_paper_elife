...
This Github repository is also linked to an OpenScience framework repository: LINK TO BE ADDED.

## Installations

In order to fit the models to data and recreate the figures, you will need to install two separate conda environments, one for R and the other for Python. These environments can be installed using the following two sets of commands:
...
The code used to fit the bifactor model can be found in the folder `fitting_bifactor_model`, and is contained in the Jupyer notebook with an R kernel `Bifactor_Fitting.ipynb`. In order to run this code, you will need to install the R conda environment using the commands above.  
...
Fitting a behavioral model to participants data can be done using code found in the `/fitting_behavioral_model_exp1` or `/fitting_behavioral_model_exp2` folders. For example, the main model (see paper for more details) can be fit by running the following python command:
